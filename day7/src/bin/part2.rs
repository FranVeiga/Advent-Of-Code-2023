...
impl Hand {
    ...
    fn get_type(&self) -> HandType {
        ...
        if let Some(js) = hmap.remove(&'J') {
            let mut keys = hmap.keys();
            let mut max_key = keys.next().unwrap_or(&'J');
            let mut max = hmap.get(max_key).unwrap_or(&5);
            for k in keys {
                if let Some(n) = hmap.get(k) {
                    if n > max {
                        max = n;
                        max_key = k;
                    }
            ...
            hmap.entry(*max_key).and_modify(|e| *e += js).or_insert(js);
        }
